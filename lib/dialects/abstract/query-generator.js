  ...
  */
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.Cast) {
      ...
      if (this.dialect === 'postgres' && ['jsonb', 'json'].includes(smth.type) && this._checkValidJsonStatement(result)) {
        const postgresJsonLiteralRegex = /(\([\S]*)#>>([\S]*\))/i;

        const functionMatches = postgresJsonLiteralRegex.exec(result);
        const [, quotedColumn, pathStr] = functionMatches;
