  ...
  */
  ...
  jsonPathExtractionQuery(column, path) {
    ...
    switch (this.dialect) {
      ...
      case 'postgres':
        const { useOtherJsonAccessOperator } = this.options;
        if (useOtherJsonAccessOperator) {
          const escapedPaths = paths.map(subPath => this.escape(subPath));
          const lastPath = escapedPaths.pop();
          return `(${[quotedColumn, ...escapedPaths].join('->')}->>${lastPath})`;
        }
